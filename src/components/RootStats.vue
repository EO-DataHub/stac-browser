  ...
  <div class="root-stats">
    ...
    <template v-if="stats">
      ...
      <dl v-for="(group, key) in stats" :key="key">
        <dt>
          {{ group.label }}
          <b-badge pill variant="primary" class="ml-1">{{
            group.count
          }}</b-badge>
          <b-badge v-if="group.version" pill variant="secondary" class="ml-1">{{
            group.version
          }}</b-badge>
        </dt>
        <dd class="charts">
  ...
  methods: {
    parseConformance(uri) {
      ...
      if (uri.startsWith("http://www.opengis.net/spec/")) {
        type = "OGC";
        let match = uri.match(
          /^http:\/\/www\.opengis\.net\/spec\/([^/]+)\/([^/]+)\/conf\/(.+)$/
        );
        if (match) {
