      ...
      <b-card-body>
        ...
        >
          <b-form-checkbox
            ...
            value="1"
            @change="setBBox()"
            >{{ $t("search.filterBySpatialExtent") }}</b-form-checkbox
          >
          <Map
            class="mb-4"
            v-if="provideBBox"
        ...
        <b-form-group
          ...
          :label="$tc('stacCollection', collections.length)"
                ...
                >
                  {{
                    $t("multiselect.andMore", {
                      count: additionalCollectionCount,
                    })
                  }}
                </strong>
              </li>
        ...
        <hr
          v-if="
            canFilterExtents ||
            conformances.CollectionIdFilter ||
            conformances.ItemIdFilter ||
...
export default {
  ...
  computed: {
    ...
    canSearchCollectionsFreeText() {
      return (
        ...
        this.supportsConformance(TYPES.Collections.FreeText)
      );
    },
    ids() {
      let obj = {};
      ["q", "datetime", "bbox", "collections", "ids", "sort", "limit"].forEach(
        (field) => (obj[field] = field + formId)
      );
      return obj;
    },
    stac() {
    ...
    showAdditionalFilters() {
      return (
        this.cql && Array.isArray(this.queryables) && this.queryables.length > 0
      );
    },
    sortOptions() {
      return [
    ...
    parent: {
      ...
      handler(newStac, oldStac) {
        if (newStac instanceof STAC) {
          newStac.setApiDataListener("searchfilter" + formId, () =>
      ...
      handler(value) {
        ...
        if (this.collections.length > 0 && this.hasAllCollections) {
          this.selectedCollections = this.collections.filter((c) =>
            query.collections.includes(c.value)
          );
        } else {
          this.selectedCollections = query.collections.map((id) => {
            let collection = this.selectedCollections.find(
              (c) => c.value === id
            );
            return collection
              ? collection
              : this.collectionToMultiSelect({ id });
          });
        }
      },
    },
  ...
  created() {
    ...
    if (this.cql && this.stac) {
      ...
      promises.push(
        this.loadQueryables(queryableLink).catch((error) =>
          console.error(error)
        )
      );
    }
    if (
      (this.type === "Collections" || this.conformances.CollectionIdFilter) &&
      this.stac
    ) {
      promises.push(
  ...
  methods: {
    ...
    searchCollections(text) {
      ...
      this.collectionsLoadingTimer = setTimeout(async () => {
        try {
          const link = Utils.addFiltersToLink(this.collectionSearchLink, {
            q: [text],
          });
          ...
          if (
            this.collectionsLoadingTimer &&
            Utils.isObject(response.data) &&
            Array.isArray(response.data.collections)
          ) {
            this.collections = this.prepareCollections(
              response.data.collections
            );
            if (typeof response.data.numberMatched === "number") {
              this.additionalCollectionCount =
                response.data.numberMatched - this.collections.length;
            }
          }
        } catch (error) {
          console.error(error);
      ...
      if (this.type === "Global" && this.collections) {
        ...
        if (!hasMore && Array.isArray(response.data.collections)) {
          let collections = response.data.collections.map(
            (collection) => new STAC(collection)
          );
    ...
    updateApiCollections() {
      ...
      if (
        !Array.isArray(apiCollections) ||
        nextCollectionsLink ||
        !this.conformances.CollectionIdFilter
      ) {
        this.collections = [];
        return;
      }
    ...
    findQueryableLink(links) {
      return Utils.getLinksWithRels(links, ogcQueryables).find((link) =>
        Utils.isMediaType(link.type, schemaMediaType, true)
      );
    },
    async loadQueryables(link) {
      ...
      if (Utils.isObject(schemas) && Utils.isObject(schemas.properties)) {
        this.queryables = Object.entries(schemas.properties).map(
          ([key, schema]) => new Queryable(key, schema)
        );
      }
    },
    sortFieldSet(value) {
    ...
    buildFilter() {
      ...
      const args = this.filters.map(
        (f) => new f.operator(f.queryable, f.value)
      );
